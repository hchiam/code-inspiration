...
function IdeasWrapper(props) {
  const [displayOptionTimestamp, setDisplayOptionTimestamp] = React.useState(
    -1
  );
  ...
  const deleteIdea = (idea) => {
    ...
    const newIdeas = props.ideas.filter(
      (e) => e.timestamp !== timestampOfIdeaToDelete
    );
  ...
  if (likelyOnMobile) {
    ...
    return (
      <div id="ideas">
        <p style={{ display: props.ideas.length > 0 ? "block" : "none" }}>
          _____________________
        </p>
        <p
          style={{
            display: props.ideas.length > 0 ? "block" : "none",
            padding: 0,
            margin: 0,
          }}
        >
